...
int main(void)
{
    ...
    auto src_image = stbi_load(src_img_path.c_str(), &src_w, &src_h, &src_c, 0);
    ...
    /* 申请输出图像内存 */
    void *dst_image = malloc((src_w - 2) * (src_h - 2) * src_c);
    ...
    const char *file_names[] = {"../demo.cl"}; // 待编译的内核文件
    ...
    error = clBuildProgram(program, 1, &device, "-cl-kernel-arg-info -cl-std=CL2.0", NULL, NULL);                                       // 编译程序
    if (error != CL_SUCCESS)
    {
    ...
    // 创建缓存对象
    cl_mem mem_src_img = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, src_c * src_h * src_w, src_image, &error); // CL_MEM_COPY_HOST_PTR指定创建缓存对象后拷贝数据
    cl_mem mem_dst_img = clCreateBuffer(context, CL_MEM_WRITE_ONLY, src_c * (src_h - 2) * (src_w - 2), NULL, &error);
    cl_mem mem_src_w = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(int), &src_w, &error);
    cl_mem mem_src_h = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(int), &src_h, &error);
    // 向内核函数传递参数
    ...
    // (w + 31) / 32 * 32
    // (h + 3) / 4 * 4
    size_t globalThreads[2] = {((src_w + localThreads[0] - 1) / localThreads[0]) * localThreads[0],
                               ((src_h + localThreads[1] - 1) / localThreads[1]) * localThreads[1]}; // 确保全局工作组完全
    ...
    clReleaseEvent(evt);
    // 读回数据
    error = clEnqueueReadBuffer(cmd_queue, mem_dst_img, CL_TRUE, 0, src_c * (src_h - 2) * (src_w - 2), dst_image, 0, NULL, NULL);
    ...
    int success = stbi_write_png("../data/output.png", src_w, src_h - 2, src_c - 2, dst_image, (src_w - 2) * (src_c - 2));
    if (success != 0)
    {
